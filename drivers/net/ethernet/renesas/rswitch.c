				    ...
				    struct rswitch_gwca_chain *c)
{
	...
	if (c->xen_frontend)
	{
		devm_kfree(&ndev->dev, c->desc);
		devm_kfree(&ndev->dev, c);
	}
}
...
static int rswitch_desc_alloc(struct rswitch_private *priv)
{
	...
	priv->desc_bat = memremap(r.start, resource_size(&r), MEMREMAP_WC);
